...
raceresults_cleaned %>%
  ...
  geom_point() +
  gghighlight(driver %in% c("Tom Dillmann", "Lucas di Grassi", "Stéphane Sarrazin", "Maximilian Günther"), label_key = driver) +
  labs(
    x = "Observed Probability of DNF",
    y = "Average Finishing Posiiton"
  )
...
raceresults_cleaned %>%
  filter(driver %in% c("Tom Dillmann","Lucas di Grassi", "Stéphane Sarrazin", "Maximilian Günther")) %>%
  ggplot(mapping = aes(
    x = driver,
    y = rank_num,
    fill = driver
